...
# un-scaled
silique_model = lmer(silique ~ 1 + partner + block + (0 + partner|RIL),
...
# scaled
silique_model = lmer(silique_std ~ 1 + partner + block + (0 + partner|RIL),
                     data = arabi_data, na.action = 'na.omit')
summary(silique_model)
varRIL = diag(VarCorr(silique_model)$RIL)
varRes = rep(attributes(VarCorr(silique_model))$sc^2, 2)
...
silique_model_D = lmer(silique_std ~ 1 + block + (1|RIL),
                       data = filter(arabi_data, partner == "D"))
summary(silique_model_D)
silique_model_S = lmer(silique_std ~ 1 + block + (1|RIL),
...
# un-scaled
weight_model = lmer(weight ~ 1 + partner + block + (0 + partner|RIL),
...
# scaled
weight_model = lmer(weight_std ~ 1 + partner + block + (0 + partner|RIL),
                    data = arabi_data, REML = FALSE, na.action = 'na.omit')
summary(weight_model)
varRIL = diag(VarCorr(weight_model)$RIL)
(h2 = varRIL)
...
# scaled per treatment
weight_model_D = lmer(weight_std ~ 1 + block + (1|RIL),
                       data = filter(arabi_data, partner == "D"))
summary(weight_model_D)
weight_model_S = lmer(weight_std ~ 1 + block + (1|RIL),
                       data = filter(arabi_data, partner == "S"))
...
# un-scaled
height_model = lmer(height ~ 1 + partner + block + (0 + partner|RIL),
                    data = arabi_data, REML = FALSE, na.action = 'na.omit')
...
# scaled
height_model = lmer(height_std ~ 1 + partner + block + (0 + partner|RIL),
                    data = arabi_data, REML = FALSE, na.action = 'na.omit')
summary(height_model)
varRIL = diag(VarCorr(height_model)$RIL)
varRes = rep(attributes(VarCorr(height_model))$sc^2, 2)
...
# scaled per treatment
height_model_D = lmer(height_std ~ 1 + block + (1|RIL),
                       data = filter(arabi_data, partner == "D"))
summary(height_model_D)
height_model_S = lmer(height_std ~ 1 + block + (1|RIL),
                       data = filter(arabi_data, partner == "S"))
