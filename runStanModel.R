...
arabi_data$silique[!mask_0]  <- log(arabi_data$silique[!mask_0] + 1)
...
bin_sim = array(0, c(replicates, sm$N_pi))
for(i in 1:replicates){
    vary_pi <- sm$vary_RIL[i, RIL_pi,1] + sm$vary_RIL[i, RIL_pi,2] * sm$partnerNONE_pi
    glm_pi <- vary_pi + sm$Intercept_pi[i] +
              sm$beta_partnerNONE_pi[i] * sm$partnerNONE_pi +
              sm$beta_block_pi[i] * sm$block_pi
...
silique_sim = array(0, c(replicates, sm$N_mu))
for(i in 1:replicates){
    vary_mu <- sm$vary_RIL[i, RIL_mu, 3] + sm$vary_RIL[i, RIL_mu,4] * sm$partnerNONE_mu
    glm_mu <- vary_mu + sm$Intercept_mu[i] +
              sm$beta_partnerNONE_mu[i] * sm$partnerNONE_mu +
              sm$beta_block_mu[i] * sm$block_mu
    glm_mu <- exp(glm_mu + 0.5 * sm$sigma[i] * sm$sigma[i])
    for ( j in 1:N_mu ) {
        silique_sim[i,j] = rlnorm(1, glm_mu[j], sm$sigma[i])
    }
}
par(mfrow=c(2, 2))
...
hist(sm$sigma); abline(v = sd(log(arabi_data$silique[!mask_0])), col = "red")

mean(silique_sim[2,])
mean(log(arabi_data$silique[!mask_0]))

qqplot(silique_sim[2,], log(arabi_data$silique[!mask_0]))
#qqline(silique_sim[1,], log(arabi_data$silique[!mask_0]))
